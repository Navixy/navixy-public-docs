...
Every custom field has a [FieldType](../api-reference/custom-fields.md#fieldtype) that determines what kind of data it stores and what validation options are available. You can add any number of fields with different types to a given entity type.
...
| `GEOJSON`   | Geometry data                                               | `isRequired`, `allowedTypes` ([GeoJsonGeometryType](../api-reference/geo-objects.md#geojsongeometrytype)) | `{ geojson: {"type":"Point","coordinates":[...]} }`                                                    |
...
If no custom fields have been created yet, `customFieldDefinitions` is an empty array. The same pattern works for [geoObjectTypes](../api-reference/geo-objects.md#geoobjecttypes), the other owner that has custom fields.
...
The same pattern applies to [geoObjectTypeUpdate](../api-reference/geo-objects.md#geoobjecttypeupdate), with just the parent type's input changing. Only `create` is allowed for a catalog item creation mutation such as [assetTypeCreate](../api-reference/assets/#assettypecreate). The full set of operations (`update`, `delete`, `archive`, `restore`) is available for [assetTypeUpdate](../api-reference/assets/#assettypeupdate).
{% endstep %}
...
The same pattern applies to [geoObjectCreate](../api-reference/geo-objects.md#geoobjectcreate).
{% endstep %}
...
The asset and geo object list queries support filtering by custom field values through [CustomFieldFilter](../api-reference/custom-fields.md#customfieldfilter). Add one or more conditions to the `customFields` filter array. Multiple conditions are applied as AND.
...
* [Custom fields](../api-reference/custom-fields.md): Complete reference for all custom field operations and types
