...
To work with a schedule, you need your workspace's ID. It comes with your access credentials and is carried in your access token. See [Authentication](../../authentication.md) for how tokens work and where the workspace ID comes from.

## How schedule data works

A schedule consists of metadata (title and workspace) and calendar data stored in the `scheduleData` field, which accepts a value of [ScheduleData](../api-reference/schedules.md#scheduledata), a structured JSON object validated on every write.
...
The maintenance provider doesn't work on public holidays. Several holidays in the year fall on Mondays. Add these as exception dates using `excludedDates`. This requires updating the schedule with [scheduleUpdate](../api-reference/schedules.md#scheduleupdate).
