...
These actions are similar with any other [API REST actions](../getting-started.md) but must be sent inside open `WebSocket` channel and use only JSON format for messages between the client and server.
...
* `hash` – required, string, length=32. Session hash code obtained by [user/auth](../resources/commons/user/README.md#post-user-auth) action.
...
> * `events` – required, [enum](../../general/api-conventions.md#data-types) array, without nulls. List of events to subscribe. An event can only be a `state`.
...
* `format` – optional, [enum](../../general/api-conventions.md#data-types), one of: "full" (default), "compact".
...
* `target` – required, [target](subscription.md#request-targets). Trackers to subscribe.
* `rate_limit` – optional, string. A timespan for batching.
* `sensor_type` – optional, [metering sensor type](../resources/tracking/tracker/sensor/README.md#metering-sensor-type-values) or [virtual sensor type](../resources/tracking/tracker/sensor/README.md#virtual-sensor-type-values).\
...
Receiver must be able to parse data from different devices in this packet.\
New data will arrive in real-time in the [event message](events.md#data-stream-analyzer-event), but no more frequently than the specified `rate_limit`.
...
* `events` – required, array of [enum](../../general/api-conventions.md#data-types), without nulls. List of the subscribed events types ("", "" or "iot\_monitor").
