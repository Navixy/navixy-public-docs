...
For efficient fuel management, especially in fleet management or cargo transportation, integrating your ERP system with the Navixy platform is essential. This integration allows you to request raw fuel data from specific devices, improving your fuel monitoring and expense tracking.
...
Please note that you will not receive human-readable information about fuel drains and refills, excessive consumption, idling, and other analytics: those are available through [backend-API methods](../getting-started.md). Raw data requests provide unprocessed data received directly from the device, decoded by the Navixy platform, offering instantaneous data readings for a specific period. This data is suitable for further processing and analysis on your end.
...
Before requesting raw data, we need to understand what data the device can report to the platform and what the data inputs are named. To do this, we need to use the [`raw_data/get_inputs`](../resources/data-warehouse-api/raw-data.md#get_inputs) request.
...
The API request [`raw_data/read`](../resources/data-warehouse-api/raw-data.md#read) for reading the required raw data in our case should look like this:
...
In the above example, we see the output for one minute of tracking. When querying raw data over a long period of time, the response can reach significant sizes. Take this into account.

In one of the lines, we see instead of fuel level and mileage values. This means that no such information was received in this data packet. The symbol represents `NULL`.
...
Similar to the [Backend API](../getting-started.md), you can specify the date and time in either the standard `YYYY-MM-DD HH:mm:ss` format with or without a time zone or in ISO 8601 format.

The default format for Raw IoT Data API requests is ISO 8601.

The platform allows you to request raw data for any period within the [time frame limits](../resources/data-warehouse-api/#time-frame-limits).
...
In this case, you need to supplement your request with the `time_zone` parameter and specify the required zone ID. You can request all the possible zone IDs using the [`timezone/list`](../resources/commons/timezone.md) request from the Backend API.
