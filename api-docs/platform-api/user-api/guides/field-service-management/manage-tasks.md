...
To create a new single task, use the [`task/create`](../../resources/field-service/task/#post-task-create) method. You need to provide a JSON object that contains all the necessary information about the task. Optionally, you can create a form for the task by setting the `create_form` parameter to `true`.
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/task/create' \
...
To create a new route task, use the [`task/route/create`](../../resources/field-service/task/route/README.md#post-task-route-create) method. You need to provide a JSON object that contains all the necessary information about the route and its checkpoints.
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/task/route/create' \
  --header 'Content-Type: application/json' \
  --data '{
        "hash": "22eac1c27af4be7b9d04da2ce1af111b",
        "route": {
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/task/route/points/optimize' \
  --header 'Content-Type: application/json' \
  --data '{
        "hash": "22eac1c27af4be7b9d04da2ce1af111b",
        "start_point": {
...
To associate a task or checkpoint with an address, include the address in the location object. Use the [`geocoder/search_location`](../../resources/tracking/geocoder.md#post-geocoder-search_location) method to obtain an address when you have a location.
