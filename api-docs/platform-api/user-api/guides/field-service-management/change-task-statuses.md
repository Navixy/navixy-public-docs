...
To create a working status list, use the `status/listing/create` method. You need to provide the `listing` parameter, which is a [status\_listing](../../resources/tracking/status/listing/README.md#status-listing-object-structure) object without the "id" and "entries" fields.
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/status/listing/create' \
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/status/create' \
  --header 'Content-Type: application/json' \
  --data '{
        "hash": "22eac1c27af4be7b9d04da2ce1af111b",
        "listing_id": 1111,
...
```shell
curl --request POST \
  --url 'https://api.eu.navixy.com/v2/status/listing/tracker/assign' \
  --header 'Content-Type: application/json' \
  --data '{
        "hash": "22eac1c27af4be7b9d04da2ce1af111b",
        "tracker_id": 615487,
