...
{% openapi-operation spec="raw-iot-data" path="/tracker/raw_data/get_inputs" method="post" %}
[OpenAPI raw-iot-data](../../reference/Raw_IoT_Data.json)
...
Additionally, you can get a description of all inputs from response using the [input\_name](../tracking/tracker/sensor/input_name.md#post-tracker-sensor-input_name-list) endpoint.
...
Retrieves parsed raw data: values received from tracking devices by the Navixy platform. Each attribute in the exported file will\
be placed in a separate column.

> The names and values of the inputs and status attributes returned by this request align with the names visible in [Air Console](https://navixy.com/docs/admin/devices/air-console) when connecting to a device. You can find them in the right column, where the incoming data is decoded.

{% openapi-operation spec="raw-iot-data" path="/tracker/raw_data/read" method="post" %}
[OpenAPI raw-iot-data](../../reference/Raw_IoT_Data.json)
{% endopenapi-operation %}
...
| event\_id      | int       | Platform related event ID. You can find the full list of event IDs [here](https://navixy.com/docs/expert-center/faq-and-troubleshooting/access-iot-data/save-iot-data-to-csv-file/columns-in-csv-file#simple-columns).                                                                                                                                                                                                                                                                                                          | 2                            |
...
device should be obtained using the [get\_inputs](raw-data.md#get_inputs) method described above.
