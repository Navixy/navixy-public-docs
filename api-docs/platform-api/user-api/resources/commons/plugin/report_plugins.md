...
description: Every report the Navixy platform can generate, with the plugin-specific parameters each one takes.
---

# Report plugins

A [report](../report/report_tracker.md) is produced by a plugin, and the plugin decides what the report contains. This page catalogues every report plugin, and for each one gives the parameters that go inside the `plugin` object of [`report/tracker/generate`](../report/report_tracker.md#post-report-tracker-generate).

Only the plugins available to the account can be used. [`plugin/list`](README.md#post-plugin-list) reports which those are, and naming one that is not available returns error 222.

Every plugin takes the common report parameters as well. Those are documented with the operation itself rather than repeated here.
...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| `show_seconds`                         | If `true`, timestamps will be with seconds.                                                                                                                                   | boolean                      |
| `graph_type`                           | The type of X-axis. Can be "time" or "mileage".                                                                                                                               | [enum](../../../../general/api-conventions.md#data-types) |
...
| `group_type`                   | A group type. Can be "province" or "country".                                                                                                                                 | [enum](../../../../general/api-conventions.md#data-types) |
...
A report on safety driving by trackers. For [report/generate](../report/report_tracker.md#post-report-tracker-generate) request use trackers parameter.
...
A report on safety driving by drivers. For [report/generate](../report/report_tracker.md#post-report-tracker-generate) request use employees parameter.
