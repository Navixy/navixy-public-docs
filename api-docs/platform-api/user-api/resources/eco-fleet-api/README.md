...
The structure of Eco Fleet API is close to the user API, so we highly recommend reading [Backend API: getting started](../../getting-started.md).
...
Authentication is handled by [Backend API](../../getting-started.md).
...
```shell
curl --request GET \
  --url 'https://api.eu.navixy.com/eco_fleet/v1/trackers/123/sensors/321/quality' \
  --header 'Authorization: NVX 5dd33ef0ab37b6aaf2064ecdf50c4cdc'
```
...
| --- | --- | --- |
| [`/trackers/{tracker_id}/resampling`](resampling.md#post-trackers-{tracker_id}-resampling) | POST | Resample fuel data |

#### Fuel sensor quality index

| Endpoint | Method | What it does |
| --- | --- | --- |
| [`/trackers/{tracker_id}/sensors/{sensor_id}/quality`](sensors/README.md#get-trackers-{tracker_id}-sensors-{sensor_id}-quality) | GET | Get fuel sensor quality index |
