---
description: >-
  Business Data Repository (BDR) manages assets, devices, geo objects, schedules,
  custom fields, and catalogs through Navixy GraphQL API.
---
...
**Business Data Repository (BDR)** is the product that manages the business entities of the Navixy platform. It is one of the products served by [Navixy GraphQL API](../README.md). It defines what you track (assets), how data is collected (devices), where important points are located (geo objects), and when things happen (schedules).
...
- [Implementing custom fields](guides/implementing-custom-fields.md), with [Custom field filtering and sorting](custom-field-filtering.md) for queries by custom field values
...
The [API reference](core-api-reference.md) provides complete technical specifications for all GraphQL types and operations of the product, grouped by category:
